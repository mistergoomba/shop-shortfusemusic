...
export const localizeGetStaticPaths = <T>(existingPaths: Array<{ params: T }>) => {
    const allPaths = getAllPossibleWithChannels();
    const paths = allPaths.flatMap(locale =>
        existingPaths.map(ep => ({ ...ep, params: { ...ep.params, ...locale.params } })),
    );
    return paths;
};
...
export function makeStaticProps(ns: Array<keyof typeof resources>) {
    return async function getStaticProps(ctx: ContextModel) {
        ...
        return { props: await getI18nProps(context, ns), context: context.params };
    };
}
